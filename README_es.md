...
* **Autocompletado inteligente:** el sistema ahorra tiempo al reutilizar los datos existentes. Por ejemplo, al registrarse para la primera comunión, extrae automáticamente los datos del bautismo de la persona, por lo que no es necesario volver a introducirlos.
...
* **Backend:** PHP (nativo/enfoque MVC)
...
### Cómo funciona
* **Reglas estrictas (restricciones):** Utiliza constraints para detener los datos erróneos antes de que entren en el sistema. Algunos ejemplos son:
...
* `chk_cedula_partida_nacimiento`: garantiza que cada feligrés tenga al menos un documento de identidad válido (documento nacional de identidad o acta de nacimiento).
...
5.  **Ejecute la comprobación del sistema:**
    ...
    * Asegúrese de que todas las luces de estado estén en verde (Librerias, Permisos, LibreOffice). Si «LibreOffice» está en rojo, la generación de PDF fallará (solo generará documentos .docx).
